...
teamTypes physicsEngine::jumpBallCollisionCheck()  // checks whether team 1 or team 2's center has made contact with the ball
{
    ...
    MyContactResultCallback jumpBallResult;
    ...
    while (x < teamInstance.size())
    {
        if (teamInstance[x].getPlayerInstancesCreated())
        {
            ...
            if (!jumpBall.getBallTipped())
            {
                world->contactPairTest(basketballInstance[activeBBallInstance].getPhysBody(), activePlayerInstance[centerID].getPhysBody(), jumpBallResult);
                logMsg("jumpBallResult.m_connected = " +convert->toString(jumpBallResult.m_connected));
                ...
                if (jumpBallResult.m_connected)
                {
                    ...
                    i = 0;
                    logMsg("jumpBall.getBallTippedToTeam == " +convert->toString(jumpBall.getBallTippedToTeam()));
//                    exit(0);
                    ...
                    jumpBall.setBallTippedToPosition(PG);
                    ...
                    while (y < activePlayerInstance.size())
                    {
                        ...
                        {
//                            gameS->set
                            jumpBall.setBallTippedToPlayerID(activePlayerInstance[y].getPlayerID());
                            break;
                        }
                        ++y;
                    }
                    ...
                    logMsg("jumpBall.setBallTippedToPosition == " +convert->toString(jumpBall.getBallTippedToPosition()));
                    jumpBall.setBallTippedToTeam(ballTippedToTeam);
...
bool physicsEngine::tippedBallCollisionCheck()  // checks if ball has collided with tipped to player
{
    ...
    std::vector<basketballs> basketballInstance = gameS->getBasketballInstance();
    size_t activeBBallInstance = gameS->getActiveBBallInstance();
    MyContactResultCallback tippedBallResult;  // callback checks if ball has collided with player it was tipped to
...
//      logMsg("activePlayerInstance size = " + convert->toString(activePlayerInstance.size()));
    ...
    logMsg("player == " +convert->toString(jumpBall.getBallTippedToPlayerID()));
    ...
    logMsg("jumpBall.getBallTippedToPlayerID() == " +convert->toString(jumpBall.getBallTippedToPlayerID()));
...
//            teamInstance[ballTippedToTeam].setPlayerWithBall(activePlayerInstance[gameS->getBallTippedToPlayer()].getPlayerID());
        ...
        {
            teamInstance[ballTippedToTeam].setHumanPlayer(jumpBall.getBallTippedToPlayerID());
...
//            exit(0);
        ...
        teamTypes teamWithBall = jumpBall.getBallTippedToTeam();
        gameS->setTeamInstance(teamInstance);
